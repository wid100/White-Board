                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input id="name" class="form-control @error('name') is-invalid @enderror"
                                            name="name" type="text" value="{{ old('name', $issue->name) }}">
                                        @error('name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                        ...
                                        <select id="year_id" name="year_id"
                                            ...
                                            @foreach ($years as $year)
                                                <option value="{{ $year->id }}"
                                                    {{ old('year_id', $issue->year_id) == $year->id ? 'selected' : '' }}>
                                                    {{ $year->year }}</option>
                                            @endforeach
                                        </select>
                                        @error('year_id')
                                        ...
                                        <select id="issue_month" name="issue_month"
                                            ...
                                            @foreach ($months as $month)
                                                <option value="{{ $month->id }}"
                                                    {{ old('issue_month', $issue->issue_month) == $month->id ? 'selected' : '' }}>
                                                    {{ $month->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('issue_month')
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <select id="editorialnote_id" name="editorialnote_id"
                                            ...
                                            @foreach ($editorialNotes as $editorialnote)
                                                <option value="{{ $editorialnote->id }}"
                                                    {{ old('editorialnote_id', $issue->editornote_id) == $editorialnote->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <textarea id="open-source-plugins" name="editorial_note"
                                            class="form-control @error('editorial_note') is-invalid @enderror" cols="30" rows="5">{{ old('editorial_note', $issue->editorial_note) }}</textarea>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        @if ($issue->image)
                                            <img src="{{ asset($issue->image) }}" alt="Current Image" width="100">
                                        @endif
                                        @error('image')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div class="mb-3">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input"
                                                {{ old('status', $issue->status) ? 'checked' : '' }} name="status"
                                                id="termsCheck">
